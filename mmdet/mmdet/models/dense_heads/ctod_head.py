...
from mmcv.ops import deform_conv2d, modulated_deform_conv2d, ModulatedDeformConv2d
...
class Cross_encoder(nn.Module):
    def __init__(self, in_channels, inter_channels = None):
        ...
        self.reduction = nn.Conv2d(self.in_channels, self.in_channels, kernel_size=1, stride=1)

    def init_weights(self):
        normal_init(self.g, std=0.01)
        normal_init(self.theta, std=0.01)
        normal_init(self.conv_out, std=0.01)
        normal_init(self.theta, std=0.01)

    def forward(self, x, y, z):
        n,_,h,w = z.shape
        g_z = self.g(z).view(n, self.inter_channels, -1)
        g_z = g_z.permute(0,2,1)
        # g_x: [N, HxW, C]
        theta_x = self.theta(x).view(n, self.inter_channels, -1)
        theta_x = theta_x.permute(0, 2, 1)
        # theta_x: [N, HxW, C]
        phi_y = self.phi(y).view(n, self.inter_channels, -1)
        inner = torch.matmul(phi_y, g_z)
        output = torch.matmul(theta_x, inner)
        output = output / (h*w)
        output = output.permute(0,2,1).reshape(n, self.inter_channels, h, w)
        output = z + self.conv_out(output)
...
class CTODHead(ATSSHead):
    ...
    def __init__(self,
        ...
        self.aggregate_point = aggregate_point
        self.epoch = 0  # which would be update in SetEpochInfoHook!
        super(CTODHead, self).__init__(num_classes, in_channels, **kwargs)
        # self.num_specific_convs = 4
    ...
    def _init_layers(self):
        ...
        for i in range(self.encoder_convs):
            if i == 0:
                chn = 2* self.feat_channels
            else:
        ...
        self.cls_prob_module_encoder = nn.Sequential(
            nn.Conv2d(self.feat_channels * self.stacked_convs,
                      self.feat_channels, 1), nn.ReLU(inplace=True))
        self.cls_prob_module = nn.Conv2d(self.feat_channels, 1, 3, padding=1)
        self.reg_offset_module_encoder = nn.Sequential(nn.Conv2d(self.feat_channels * self.stacked_convs,
                      self.feat_channels, 1), nn.ReLU(inplace=True))
        self.reg_offset_module = nn.Conv2d(self.feat_channels, 4 * 2, 3, padding=1)

        self.scales = nn.ModuleList(
            [Scale(1.0) for _ in self.prior_generator.strides])

        self.offset_cls = nn.Conv2d(
            self.feat_channels, self.aggregate_point * 3, 3, padding=1)
        self.offset_reg = nn.Conv2d(
            self.feat_channels, self.aggregate_point * 3, 3, padding=1)

        self.cls_cross_encoder = Cross_encoder(in_channels = self.feat_channels)
        self.reg_cross_encoder = Cross_encoder(in_channels = self.feat_channels)

        self.cls_SAFG = nn.ModuleList()
        self.reg_SAFG = nn.ModuleList()
        for i in range(self.aggregate_point):
            self.cls_SAFG.append(ModulatedDeformConv2d(
                self.feat_channels,
                self.feat_channels,
                1,
                stride=1,
            ...
            self.reg_SAFG.append(ModulatedDeformConv2d(
                self.feat_channels,
                self.feat_channels,
                1,
                stride=1,
    ...
    def init_weights(self):
        ...
        for m in self.cls_prob_module_encoder:
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.01)
        normal_init(self.cls_prob_module, std=0.01, bias=bias_cls)

        for m in self.reg_offset_module_encoder:
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001)
        ...
        self.cls_cross_encoder.init_weights()
        self.reg_cross_encoder.init_weights()
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            cross_cls_feat = torch.cat((self.cls_prob_module_encoder(feat), cls_feat), 1)
            cross_reg_feat = torch.cat((self.reg_offset_module_encoder(feat), reg_feat), 1)
            for cls_conv in self.cls_convs:
                cross_cls_feat = cls_conv(cross_cls_feat)
            for reg_conv in self.reg_convs:
                cross_reg_feat = reg_conv(cross_reg_feat)

            cls_embedding = self.cls_cross_encoder(cross_reg_feat, cross_cls_feat, cross_cls_feat)
            reg_embedding = self.reg_cross_encoder(cross_cls_feat, cross_reg_feat, cross_reg_feat)

            cls_offset_map = self.offset_cls(cls_embedding)
            reg_offset_map = self.offset_reg(reg_embedding)

            cls_offset, cls_mask = cls_offset_map.split(2 * self.aggregate_point, dim=1)
            reg_offset, reg_mask = reg_offset_map.split(2 * self.aggregate_point, dim=1)

            # cls prediction and alignment
            for i in range(self.aggregate_point):
                cls_feat_i = self.cls_SAFG[i](cls_feat, cls_offset[:,i:i+2,...].contiguous(), cls_mask[:,i:i+1,...].sigmoid())
                reg_feat_i = self.reg_SAFG[i](reg_feat, reg_offset[:,i:i+2,...].contiguous(), reg_mask[:,i:i+1,...].sigmoid())
                if i == 0:
                    cls_feat_out = cls_feat_i
                    reg_feat_out = reg_feat_i
                else:
                    cls_feat_out = cls_feat_out + cls_feat_i
                    reg_feat_out = reg_feat_out + reg_feat_i
            cls_logits = self.tood_cls(cls_feat_out)
            # reg prediction and alignment
            if self.anchor_type == 'anchor_free':
                reg_dist = scale(self.tood_reg(reg_feat_out).exp()).float()
                ...
                reg_dist = scale(self.tood_reg(reg_feat_out)).float()
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 4)
            ...
            cls_prob = self.cls_prob_module(cls_embedding)
            cls_score = sigmoid_geometric_mean(cls_logits, cls_prob)
            reg_offset = self.reg_offset_module(reg_embedding)
    ...
    def deform_offset(self, feat, offset, mask):
        b, c, h, w = feat.shape
        weight = feat.new_ones(c, 1, 1, 1)
        y = modulated_deform_conv2d(feat, offset, mask, weight)
        return y
